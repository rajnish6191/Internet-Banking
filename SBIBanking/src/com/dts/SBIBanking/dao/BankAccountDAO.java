...
public class BankAccountDAO extends AbstractDataAccessObject {
	...
	public boolean register(Profile aprofile)
	{
		...
		//n=1000+n;
			PreparedStatement pst=con.prepareStatement("insert into bankaccounts values(?,?,?,?,?,?)");
...
if(n1>0)
{
	...
	public Profile viewAccountNos(int accountno)
	{
		...
		try{
			con=getConnection();
			PreparedStatement pst=con.prepareStatement("select *	from bankaccounts where accountno=?");
...
while(rs.next())
{
	...
	aprofile.setAccountOpeningDate(DateWrapper.parseDate(rs.getDate(5)));
	...
	public Profile viewAccountNos(String loginid)
	{
		...
		try{
			con=getConnection();
			PreparedStatement pst=con.prepareStatement("select * from bankaccounts where loginid=?");
...
if(rs.next())
{
	...
	public boolean update(CustomerTransactionModel amodel)
	{
		...
		try{
			...
			System.out.println(amodel.getAmount()+" beans "+amodel.getAccountno());
	...
	public boolean updateFundsTransfer(FundsTransferModel amodel)
	{
		...
		try{
         ...
         ResultSet rs=pst.executeQuery("select AccountActivebalance from bankaccounts where AccountNo="+amodel.getFromAccountno());
        double getbal=0;
         while(rs.next()){
         ...
         {
         pst.addBatch("update bankaccounts set AccountActivebalance=AccountActivebalance-"+amodel.getAmount()+" where AccountNo="+amodel.getFromAccountno());
         pst.addBatch("update bankaccounts set AccountActivebalance=AccountActivebalance+"+amodel.getAmount()+" where AccountNo="+amodel.getToAccountno());			
...
String accountno="";
		try{
			con=getConnection();
			PreparedStatement pst=con.prepareStatement("select accountno from bankaccounts where loginid=?");
