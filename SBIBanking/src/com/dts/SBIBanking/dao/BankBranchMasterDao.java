...
import com.dts.SBIBanking.model.*;
	...
	public class BankBranchMasterDao extends AbstractDataAccessObject 
	{
		public boolean insertBankBranchMaster(BankBranchMasterForm bbform)
		{
			...
			con=getConnection();
			PreparedStatement ps=con.prepareStatement("insert into BankBranchMaster values(?,?,?,?,?,?)");
			ps.setInt(1,n);
	...
	{   CoreList v=new CoreList();
		...
		try
		{
			con=getConnection();
			Statement st=con.createStatement();
			ResultSet rs=st.executeQuery("select branchid,BRANCHADDRESS from BankBranchMaster");
	...
	{   BankBranchMasterForm bbform=new BankBranchMasterForm();
		...
		try
		{
			...
			PreparedStatement ps=con.prepareStatement("select * from BankBranchMaster where BranchId = ? ");
	...
	public boolean deleteBankBranchMaster(int BranchId)
		{
			...
			try
			{
				...
				PreparedStatement ps=con.prepareStatement("delete from BankBranchMaster where BranchId = ? ");
				ps.setInt(1,BranchId);
				int i=ps.executeUpdate();
				if(i>0)
					flag=true;
	...
	public boolean UpdateBankBranchMaster(BankBranchMasterForm bbform)
	{
		...
		con=getConnection();
		PreparedStatement ps=con.prepareStatement("update BankBranchMaster set BranchAddress = ?,BranchPhoneno1 = ?, BranchPhoneno2 = ?,BranchPhoneno3 = ?,Email = ? where BranchId = ? ");
